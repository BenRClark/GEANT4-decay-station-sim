...
void analysis_comparison(){
  ...
  for(int i = 0; i < numcrystals; i++){
    ...
    infile.open(Form("New_Results/Clover_%i_Results.txt",i));

    if(!infile.is_open()) {
      cout << "File " << Form("New_Results/Clover_%i_Results.txt",i) << " could not be opened, moving on....\n" ;
    } else {
  ...
  double Peak_Efficiency_crystal_origin[simnumE][numcrystals] = {};
  ...
  double Total_Efficiency_crystal_origin[simnumE][numcrystals] = {};
  ...
  for(int i=0; i<simnumE; i++) {
    ...
    f1t[i] = new TFile(Form("/data/e17011/simulations/analysis/simrootfiles/%dkeVtest_brc339_0.root",Energy[i]));
  }
  ...
  TFile *fout = new TFile("/data/e17011/simulations/analysis/comparison/test_brc339.root","RECREATE");
  ...
  for(int i = 0; i < numcrystals; i++){
	...
	//populating downstream efficiency and error vectors
	simdowneff_Full[j] += Peak_Efficiency_crystal_origin[j][i];
	simdownefferr_Full[j] += pow(Peak_Efficiency_Err_crystal_origin[j][i],2);
      }
      //checking to see if the crystal is upstream
      if(std::find(simupcrystals.begin(), simupcrystals.end(), i) != simupcrystals.end()) {
	//populating upstream efficiency and error vectors
	simupeff_Full[j] += Peak_Efficiency_crystal_origin[j][i];
	simupefferr_Full[j] += pow(Peak_Efficiency_Err_crystal_origin[j][i],2);
      }
      //checking to see if the crystal is on the right
      if(std::find(simrightcrystals.begin(), simrightcrystals.end(), i) != simrightcrystals.end()) {
	//populating right efficiency and error vectors
	simrighteff_Full[j] += Peak_Efficiency_crystal_origin[j][i];
	simrightefferr_Full[j] += pow(Peak_Efficiency_Err_crystal_origin[j][i],2);
      }
      //checking to see if the crystal is on the left
      if(std::find(simleftcrystals.begin(), simleftcrystals.end(), i) != simleftcrystals.end()) {
	//populating left efficiency and error vectors
	simlefteff_Full[j] += Peak_Efficiency_crystal_origin[j][i];
	simleftefferr_Full[j] += pow(Peak_Efficiency_Err_crystal_origin[j][i],2);
      }
      //checking to see if the crystal is above
      if(std::find(simabovecrystals.begin(), simabovecrystals.end(), i) != simabovecrystals.end()) {
	//populating above efficiency and error vectors
	simaboveeff_Full[j] += Peak_Efficiency_crystal_origin[j][i];
	simaboveefferr_Full[j] += pow(Peak_Efficiency_Err_crystal_origin[j][i],2);
      }
      //checking to see if the crystal is below
      if(std::find(simbelowcrystals.begin(), simbelowcrystals.end(), i) != simbelowcrystals.end()) {
	//populating below efficiency and error vectors
	simbeloweff_Full[j] += Peak_Efficiency_crystal_origin[j][i];
	simbelowefferr_Full[j] += pow(Peak_Efficiency_Err_crystal_origin[j][i],2);
      }
    }
  ...
  cout << "Printing out Sum_Eff_Down_corr" << endl;
  for(int i = 0; i < numE; i++){
      cout << Sum_Eff_Down_corr[i] << endl;
    }
  ...
  gre2->SetTitle("Simulation");
  gre3->SetTitle("Simulation");
  gre4->SetTitle("Simulation");
  gre5->SetTitle("Simulation");
  gre6->SetTitle("Simulation");
  gre7->SetTitle("Simulation");
  gre8->SetTitle("Simulation");

  grtot->SetTitle("Fit Data");
  grdow->SetTitle("Fit Data");
  grup->SetTitle("Fit Data");
  grrig->SetTitle("Fit Data");
  grlef->SetTitle("Fit Data");
  grabo->SetTitle("Fit Data");
  grbel->SetTitle("Fit Data");
  ...
  totmg->Draw("ap");
  totmg->Write();
  Total->BuildLegend(0.8,0.8,0.9,0.9);
