...
#include "e18016_simAnalysisExample.hh"
#include "e18016_simAnalysisExampleMessenger.hh"

#include "e18016_simDetectorHit.hh"
//#include "e18016_simscintHit.hh"
//#include "e18016_simPMTHit.hh"
...
e18016_simAnalysisExample::e18016_simAnalysisExample(){

  //cout<<"e18016_simAnalysisExample"<<endl;
  analysisexample = new e18016_simAnalysisExampleMessenger(this);
  ...
  e18016_simtree = (TTree *)0; // e18016_sim out tree
...
void e18016_simAnalysisExample::Construct(const G4VPhysicalVolume */*theWorldWolume*/)
{
  // cout<<"e18016_simAnalysisExample Construct"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
  if (gSystem) gSystem->ProcessEvents();
...
void e18016_simAnalysisExample::ConstructParticle()
{
  //  cout<<"e18016_simAnalysisExample ConstructParticle"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::ConstructProcess()
{
  //  cout<<"e18016_simAnalysisExample ConstructProcess"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::SetCuts()
{
  //  G4cout<<"e18016_simAnalysisExample SetCuts"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::GeneratePrimaries(const G4Event *anEvent,G4double beam)
{
  // G4cout<<"e18016_simAnalysisExample GeneratePrimaries"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
  if (gSystem) gSystem->ProcessEvents();
...
//void e18016_simAnalysisExample::ResetStartLocation(G4double &newx, G4double &newy)
//{
  // #if defined (G4ANALYSIS_USE_ROOT)
  //   if (gSystem) gSystem->ProcessEvents();
...
void e18016_simAnalysisExample::DetectorInfo(e18016_simDetectorConstruction* myDet){
  ...
  e18016_siminfo = (TTree *)gROOT->FindObject("e18016_siminfo");
  if(e18016_siminfo);
  else{
    e18016_siminfo = new TTree("e18016_siminfo","e18016_siminfo variables");
    e18016_siminfo->Branch("UseGeThickDetector",&UseGeThickDetector,"UseGeThickDetector/I");
    e18016_siminfo->Branch("UseGeThinDetector",&UseGeThinDetector,"UseGeThinDetector/I");
    e18016_siminfo->Branch("GeThickDetectorThickness",&GeThickDetectorThickness,"GeThickDetectorThickness/D");
    e18016_siminfo->Branch("GeThinDetectorThickness",&GeThinDetectorThickness,"GeThinDetectorThickness/D");
    e18016_siminfo->Branch("GeThickDetectorRadius",&GeThickDetectorRadius,"GeThickDetectorRadius/D");
    e18016_siminfo->Branch("GeThinDetectorRadius",&GeThinDetectorRadius,"GeThinDetectorRadius/D");
    e18016_siminfo->Branch("GeThickx",&GeThickx,"GeThickx/D");
    e18016_siminfo->Branch("GeThicky",&GeThicky,"GeThicky/D");
    e18016_siminfo->Branch("GeThickz",&GeThickz,"GeThickz/D");
    e18016_siminfo->Branch("GeThinx",&GeThinx,"GeThinx/D");
    e18016_siminfo->Branch("GeThiny",&GeThiny,"GeThiny/D");
    e18016_siminfo->Branch("GeThinz",&GeThinz,"GeThinz/D");
    e18016_siminfo->Branch("GeSpacing",&GeSpacing,"GeSpacing/D");
  }
  e18016_siminfo->Fill();
...
void e18016_simAnalysisExample::BeginOfRunAction(const G4Run */*aRun*/, G4int rank)
{
  //  cout<<"e18016_simAnalysisExample BeginOfRunAction"<<endl;
  eventcounter = 0;
  ...
  e18016_simtree = (TTree *)gROOT->FindObject("e18016_simtree");
  if(e18016_simtree);
  else
    {
      e18016_simtree = new TTree("e18016_simtree","DSSD hit variables");
      e18016_simtree->Branch("e18016_sim_energy",&e18016_sim_energy,"e18016_sim_energy/D");
      e18016_simtree->Branch("totE",&totE,"totE/D");
      e18016_simtree->Branch("beam_energy",&beam_energy,"beam_energy/D");
      e18016_simtree->Branch("pathlength",&pathlength,"pathlength/D");
      e18016_simtree->Branch("totaledep",&totaledep,"totaledep/D");
      e18016_simtree->Branch("tedepx",&tedepx,"tedepx[20]/D");
      e18016_simtree->Branch("tedepy",&tedepy,"tedepy[20]/D");
 ...
 //BC - Si Scintillator histograms
  //just a single one for now, which represents dynode signal
  hSiScint = (TH1D *)gROOT->FindObject("hSiScint");
  if (hSiScint) hSiScint->Reset();
  else{
    hSiScint = new TH1D("hSiScint","hSiScint", 3000, 0, 3000);
  }

  hposSi = (TH2D *)gROOT->FindObject("hposSi");
  if (hposSi) hposSi->Reset();
  else{
    hposSi = new TH2D("hposSi","hposSi", 160, -20, 20, 160, -20, 20);
  }

  hposSi3D = (TH3D *)gROOT->FindObject("hposSi3D");
  if (hposSi3D) hposSi3D->Reset();
  else{
    hposSi3D = new TH3D("hposSi3D","hposSi3D", 160, -20, 20, 160, -20, 20,50,-1.0,1.0);
  }
...
void e18016_simAnalysisExample::EndOfRunAction(const G4Run */*aRun*/, G4int rank)
{
  ...
  //file1 = new TFile(filenam.c_str(),"RECREATE");
  //e18016_siminfo->SetDirectory(file1);
  file1->cd();
  gROOT->SetStyle("Pub");
  e18016_siminfo->Write();

  //e18016_simtree->SetDirectory(file1);
  e18016_simtree->Write();
...
void e18016_simAnalysisExample::BeginOfEventAction(const G4Event */*anEvent*/)
{
  // cout<<"e18016_simAnalysisExample BeginOfEventAction"<<endl;
...
void e18016_simAnalysisExample::EndOfEventAction(const G4Event *evt)
{
  ...
  G4int CloverColID2 = SDman->GetCollectionID(colNam="CloverCollection2");
 ...
 //BC - add int that identifies the Si
  G4int SiScintColID = SDman->GetCollectionID(colNam="SiScintCollection");

  G4HCofThisEvent *HCE = evt->GetHCofThisEvent();
  e18016_simDetectorHitsCollection *DHC = 0;
  e18016_simDetectorHitsCollection *DHCthin = 0;
  e18016_simDetectorHitsCollection *DHCsega = 0;
  e18016_simDetectorHitsCollection *DHCClover = 0;
  e18016_simDetectorHitsCollection *DHCClover2 = 0;
  //BC - add LaBr3 to detector hits collection
  e18016_simDetectorHitsCollection *DHClabr3 = 0;
  //BC - add Getest to detector hits collection
  e18016_simDetectorHitsCollection *DHCGetest = 0;
  //BC - add EXOtest to detector hits collection
  e18016_simDetectorHitsCollection *DHCEXOtest = 0;
  //BC - add EXOtest to detector hits collection
  e18016_simDetectorHitsCollection *DHCEJ204Scint = 0;
  e18016_simDetectorHitsCollection *DHCCeBr3Scint = 0;
 e18016_simDetectorHitsCollection *DHCSiScint = 0;
  ...
  if(HCE){
    ...
    DHCthin = (e18016_simDetectorHitsCollection*)(HCE->GetHC(GeThinDetectorColID));
    DHCsega = (e18016_simDetectorHitsCollection*)(HCE->GetHC(SegaDetectorID));
    DHCClover = (e18016_simDetectorHitsCollection*)(HCE->GetHC(CloverColID));
    DHCClover2 = (e18016_simDetectorHitsCollection*)(HCE->GetHC(CloverColID2));
    //BC - handle HC (hit collection) for LaBr3
    DHClabr3 = (e18016_simDetectorHitsCollection*)(HCE->GetHC(LaBr3ColID));
    //BC - handle hit collection for Getest
    DHCGetest = (e18016_simDetectorHitsCollection*)(HCE->GetHC(GetestColID));
    DHCEXOtest = (e18016_simDetectorHitsCollection*)(HCE->GetHC(EXOtestColID));
    DHCEJ204Scint = (e18016_simDetectorHitsCollection*)(HCE->GetHC(EJ204ScintColID));
    DHCCeBr3Scint = (e18016_simDetectorHitsCollection*)(HCE->GetHC(CeBr3ScintColID));
 DHCSiScint = (e18016_simDetectorHitsCollection*)(HCE->GetHC(SiScintColID));
  }
  ...
  //BC - hits in Si scintillator
  {
    
    int n_hit = DHCSiScint->entries();
    
    //G4cout << "n_hit = " << n_hit << endl;
    G4double SiScintE = 0;
    G4double SiScintposx = 0;
    G4double SiScintposy = 0;
    G4double SiScintposz = 0;
    G4ThreeVector sipos;
    
    if(n_hit > 0){
      for(int i=0; i<n_hit; i++){
        G4String name = (*DHCSiScint)[i]->GetParticlename();
        G4String parentname = (*DHCSiScint)[i]->GetParentName();
        //G4double parentenergy = (*DHCSiScint)[i]->GetParentEnergy();
        G4double edep = (*DHCSiScint)[i]->GetEdep()/keV;
        G4ThreeVector pos = (*DHCSiScint)[i]->GetPostPosition();
	sipos = pos;
        G4int copyno = (*DHCSiScint)[i]->GetVolCopyNo();
        SiScintE += edep;
      }

      if(SiScintE > 0){
     
	SiScintposx = G4double(sipos.x()/mm);
	SiScintposy = G4double(sipos.y()/mm);
	SiScintposz = G4double(sipos.z()/mm);
        
	//CeBr3ScintE = gRandom->Gaus(SiScintE,(resolution*CeBr3ScintE/2.355)); need to make silicon
      ...
      //fill histogram
      hSiScint->Fill(SiScintE);
      hposSi->Fill(SiScintposx, SiScintposy);
      hposSi3D->Fill(SiScintposx, SiScintposy, SiScintposz);
    }
  ...
  if(DHCClover){
    ...
    //int n_hit2 = DHCClover2->entries();
    //double nh2 = DHCClover2->entries();
    //clover_hit=clover_hit+nh;
    if((DHCClover)/*&&(n_hit<25)*/){     /*cout << "found clover event " << n_hit << endl;*/
...
if(DHCClover2){
    ...
    int n_hit = DHCClover2->entries();
    //double nh = DHCClover->entries();
    //double nh2 = DHCClover2->entries();
    //clover_hit=clover_hit+nh;
    if((DHCClover2)/*&&(n_hit<25)*/){     /*cout << "found clover event " << n_hit << endl;*/
      // cout <<n_hit<<" crystal numbers: ";
      // cout <<n_hit<<endl;
      for(int i=0; i<(n_hit); i++){
	      G4String name = (*DHCClover2)[i]->GetParticlename();
	      G4double edep = (*DHCClover2)[i]->GetEdep()/keV;
	      G4ThreeVector pos = (*DHCClover2)[i]->GetPostPosition();
	      G4int copyno = (*DHCClover2)[i]->GetVolCopyNo();
	      G4int parentno = (*DHCClover2)[i]->GetParentno();
	      G4int trackno = (*DHCClover2)[i]->GetTrackno();
	      //cout<<"name+track"<<name <<trackno<<" parent "<<parentno<<" crys "<<copyno<<" edep "<<edep<<endl;
	      //cout<<name<<" "<<edep<<endl;
	      /*	if(name=="e-" && parentno==1){//makes no difference
	      //cout <<"here"<<endl;
...
void e18016_simAnalysisExample::ClassifyNewTrack(
...
void e18016_simAnalysisExample::NewStage()
{
  // cout<<"e18016_simAnalysisExample NewStage"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::PrepareNewEvent()
{
  //  cout<<"e18016_simAnalysisExample PreparNewEvent"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::PrepareNewRun(G4double x, G4double y, G4double z, G4double zpos,
					     G4int r, G4int c)
{
  // cout<<"e18016_simAnalysisExample PrepareNewRun"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::PreUserTrackingAction(const G4Track */*aTrack*/)
{
  //   cout<<"e18016_simAnalysisExample PreUserTrackingAction"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
  if (gSystem) gSystem->ProcessEvents();
...
void e18016_simAnalysisExample::PostUserTrackingAction(const G4Track */*aTrack*/,
						      G4TrackStatus */*status_ptr*/)
{
  // cout<<"e18016_simAnalysisExample PostUserTrackingAction"<<endl;
...
void e18016_simAnalysisExample::UserSteppingAction(const G4Step *aStep)
{
  //  cout<<"e18016_simAnalysisExample UserSteppingAction"<<endl;
#if defined (G4ANALYSIS_USE_ROOT)
...
void e18016_simAnalysisExample::OnceAWhileDoIt(const G4bool DoItNow)
{
  //  cout<<"e18016_simAnalysisExample OnceAWhileDoIt"<<endl;
  ...
  const char OnceAWhileCanvas[] = "e18016_simAnalysisExampleCanvas";
...
// end of file e18016_simAnalysisExample.cc by Jacek M. Holeczek

void e18016_simAnalysisExample::Setfilename(G4String nam)
{
  filename = nam;
}

void e18016_simAnalysisExample::Setdirname(G4String nam)
{
  dirname = nam;
}

//void e18016_simAnalysisExample::OpticalPhotons(G4int status, G4double val)
//{
  //   if(status == 1){
  //     opticalphoton++;
  //   }
...
//void e18016_simAnalysisExample::PSF(G4int status, G4double val1, G4double val2)
//{
  
  //   if(status == 0){
  //     e18016_simap_len = e18016_simap_len + val1;
  //   }
  //   else if(status == 1){
  //     psfx = val1;
  //     psfy = val2;
  //     htally->Fill(psfx,psfy);
  //     hpsf->Fill(psfx,psfy,exp(-e18016_simap_len/5));
  //   }
