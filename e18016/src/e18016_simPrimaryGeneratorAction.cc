...
#include "TRandom.h"
#include "e18016_simPrimaryGeneratorAction.hh"
...
G4GeneralParticleSource* particleGunG;
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....



e18016_simPrimaryGeneratorAction::e18016_simPrimaryGeneratorAction()
{
  // G4cout<<"e18016_simPrimaryGeneratorAction"<<G4endl;
  particleGunG = new G4GeneralParticleSource ();
}
...
void e18016_simPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  // G4cout<<"GeneratePrimaries"<<G4endl;
  particleGunG->GeneratePrimaryVertex(anEvent);
}
...
{
  // G4cout<<"e18016_simPrimaryGeneratorAction"<<G4endl;
  //TRandom *benRandom = new TRandom();
   particleGun = new G4ParticleGun(1);
   //G4ParticleDefinition* particle = gamma; 
   // particleGun->SetParticleDefiniton(particle);
  //G4double position_x = 10*mm;//10*benRandom->Gaus(0.0,1)*mm;
  //G4double position_y = 10*mm;//(benRandom->Gaus(0.0,1))*mm;
  //std::cout << position_x<< std::endl;
  //G4double position_z = (G4UniformRand()-0.5)*mm;
  //particleGun->SetParticlePosition(G4ThreeVector(position_x,position_y,position_z));
  
  //particleGun->G4ThreeVector
}
...
void e18016_simPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  G4double position_x = (G4INCL::Random::gauss(6.67))*mm;
  G4double position_y = (G4INCL::Random::gauss(6.67))*mm;
  G4double position_z = (G4UniformRand()-0.5)*mm;
  //std::cout << position_x<< " " << position_y << " " << position_z << std::endl;
  particleGun->SetParticlePosition(G4ThreeVector(position_x,position_y,position_z));
  particleGun->SetParticleMomentumDirection(G4ThreeVector((G4UniformRand()-0.5)*m,(G4UniformRand()-0.5)*m,(G4UniformRand()-0.5)*m));
...
void e18016_simPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  position_X = gRandom->Gaus(1.0,6.67)*cm; //was .75 on both
  // position_X = 0.25*cm;
  position_Y = gRandom->Gaus(1.0,6.67)*cm;
  // position_Y = 0.25*cm;
  // position_Z = gRandom->Uniform(-0.500,-0.490)*cm;  //0 to 100 microns deep
  // position_Z = gRandom->Uniform(-0.495,-0.485)*cm;  //50 to 150 microns deep
  // position_Z = gRandom->Uniform(-0.490,-0.480)*cm;  //100 to 200 microns deep
  ...
  // position_Z = gRandom->Uniform(-0.465,-0.455)*cm;  //350 to 450 microns deep
  // position_Z = gRandom->Uniform(-0.460,-0.450)*cm;  //400 to 500 microns deep
   position_Z = gRandom->Uniform(0.0,1.0)*cm;  //900 to 1000 microns deep
