...
//   //creation of BCS beam vac
 ...
 /* //   // creation of BCS detector
  ...
  G4double CloverDeadLayerLength = 0.22*mm;
  G4double CloverDeadLayerRadius = 0.22*mm;
...
//     /**** end explode geometry ****/  
    ...
    G4double upOffset = -0.8*cm;
    G4double downOffset = -0.1*cm;
	...
	CloverCryoPos = G4ThreeVector((distclover-Offset+upOffset)*sin(pi/4*ii)*sin(0) , (distclover-Offset+upOffset)*sin(pi/4*ii)*cos(0)+yOffset, (distclover-Offset+upOffset)*cos(pi/4*ii) + zOffset);
      ...
      if(ii!=0 && ii!=4){
	if(count==1 || count == 7 || count == 13 || count == 17){
	  ...
	  if(count==11){
	   CloverCryoPos = G4ThreeVector((distclover-(0.6*cm)+upOffset)*sin(pi/4*ii)*sin(0) , (distclover-(0.6*cm)+upOffset)*sin(pi/4*ii)*cos(0)+yOffset, (distclover-(0.6*cm)+upOffset)*cos(pi/4*ii) + zOffset);
	   }
	  physiCloverCryo = new G4PVPlacement(rotate_ring2[ii],
					       CloverCryoPos,
					       logicCloverCryo,
    ...
    count=14;
    for(int ii = 0; ii < 8; ii++){
      CloverCryoPos = G4ThreeVector(distclover*cos(pi/4*ii), distclover*sin(pi/4*ii)*cos(0)+yOffset, distclover*sin(pi/4*ii)*sin(0) + zOffset);
    if(ii!=0 && ii!=2 && ii!=4 && ii!=6){
    	if(count==1 || count == 7 || count == 8 || count == 13){
...
//((CloverBracketLength/2.0)+(TriangleLength/(2.0+(2.0*sqrt(3)))));
  ...
  */
  ...
  e17011_simDetectorSD* aCloverDetectorSD2 = new e17011_simDetectorSD( detectorSDname);
